'use client';
import { Download, ArrowLeft, Utensils, UserPlus } from "lucide-react";
...
export default function ManifestContent({ busId, onBack, allowWalkIn = false }: ManifestContentProps) {
  ...
  if (!currentTripPassengers.length) {
    // Use toast instead of alert during render
    toast.info("No passenger data to export.");
    return (
      <div className="max-w-6xl mx-auto my-8 px-4">
        <button
          onClick={() => (typeof onBack === "function" ? onBack() : router.push("/admin?tab=schedule"))}
          className="flex items-center gap-2 text-sm font-medium text-teal-600 hover:text-teal-700 transition-colors w-fit mb-4"
        >
          <ArrowLeft className="w-4 h-4" />
          Back to Bus Schedule
        </button>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-center">
          <h2 className="text-xl font-bold text-slate-900 mb-2">No Passengers</h2>
          <p className="text-slate-600">This trip has no passenger bookings yet.</p>
        </div>
      </div>
    );
  }
