...
export function QuickAddPassengerModal({ isOpen, onClose, trip, bookings, onSuccess }: QuickAddPassengerModalProps) {
  ...
  useEffect(() => {
    if (isOpen && trip?.id) {
      ...
      fetch(`/api/trips/${trip.id}/bookings`)
        .then(res => res.json())
        .then(data => {
          const manualRaw = trip.occupiedSeats ? JSON.parse(trip.occupiedSeats) : [];
          const manual = manualRaw.map((s: string) => normalizeSeat(String(s)));
          ...
          const tempLocked = data.trip?.tempLockedSeats ? data.trip.tempLockedSeats.split(',').map((s: string) => normalizeSeat(s.trim())) : [];
  ...
  const renderSeatButton = (seatId: number) => {
    ...
    const isOccupied = unavailable.has(seatStr);
    const isSelected = passengers.some(p => p.seatNumber === seatStr);
    const isTempLocked = occupiedDetails.tempLocked.includes(seatStr);
    return (
      <button
        key={seatId}
        disabled={isOccupied && !isSelected}
        onClick={() => isSelected ? null : handleSeatSelect(seatStr)}
        className={cn(
                        ...
                        {row.isBack ? (
                          ...
                          <>
                            <div className="flex gap-1.5">
                              {row.isWC ? (
                                <div className="w-[86px] h-10 rounded-lg bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-300">
                                  WC
                                </div>
                              ) : (
